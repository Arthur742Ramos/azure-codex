...
#[derive(Clone)]
pub(crate) enum AzureSetupState {
...
pub(crate) struct AzureSetupWidget {
...
impl AzureSetupWidget {
    ...
    fn render_endpoint_entry(&self, area: Rect, buf: &mut Buffer) {
        ...
        if let Ok(error) = self.error.read() {
    ...
    fn render_model_selection(&self, area: Rect, buf: &mut Buffer) {
        ...
        let mut lines: Vec<Line> = vec![
            ...
            "".into(),
            "  Available GPT deployments:".into(),
            "".into(),
        ];
    ...
    fn save_config(&self) {
        ...
        if let Some(model_name) = model.clone() {
            // Save to config
            let result = ConfigEditsBuilder::new(&self.codex_home)
                .with_edits(vec![
                    ConfigEdit::SetPath {
                        segments: vec!["azure_endpoint".to_string()],
                        value: value(endpoint.clone()),
                    },
                    ConfigEdit::SetPath {
                        segments: vec!["model".to_string()],
                        value: value(model_name.clone()),
                    },
                ])
                .apply_blocking();
