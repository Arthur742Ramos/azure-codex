...
impl AzureSetupWidget {
    ...
    fn render_model_selection(&self, area: Rect, buf: &mut Buffer) {
        // Constrain the width to avoid rendering issues on narrow terminals
        const MAX_CONTENT_WIDTH: u16 = 80;
        let content_width = area.width.min(MAX_CONTENT_WIDTH);
        let content_area = Rect {
            x: area.x,
    ...
    fn render_configuring(&self, area: Rect, buf: &mut Buffer) {
        ...
        let lines: Vec<Line> = vec![
            ...
            "".into(),
            Line::from(vec!["  Endpoint: ".dim(), endpoint.clone().cyan()]),
            Line::from(vec!["  Model: ".dim(), model_name.cyan()]),
        ];
    ...
    fn save_config(&self) {
        // Transition to configuring state immediately for visual feedback
        *self.state.write().unwrap() = AzureSetupState::Configuring;
        ...
        if let Some(model_name) = model {
            ...
            tokio::spawn(async move {
                // Brief delay to show the configuring state
...
"#
            ...
            // No model selected, go back to model selection
    ...
    fn handle_key_event(&mut self, key_event: KeyEvent) {
        ...
        match state {
            ...
            AzureSetupState::ModelSelection => {
                // Filter out non-press events to prevent double-jump on press+release
                if key_event.kind != KeyEventKind::Press {
                    return;
                }
            ...
            AzureSetupState::Configuring | AzureSetupState::Complete | AzureSetupState::Skipped => {
